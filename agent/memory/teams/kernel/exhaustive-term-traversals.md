---
scope: teams/kernel
audience: kernel-implementer, kernel-qa, kernel-leader (any WP adding a Term
  variant)
source: private memory `zonk_term-must-be-exhaustive-over-term-variants`
...
`kernel_check` unresolved, surfacing as
`TypeMismatch{expected: Type u2, found: Type 0}` — but only on the genuinely
universe-polymorphic case (`cast`'s `Eq Type A B`), where an unresolved level
actually diverges from the harmless Zero-default.
...
2. The fix is to make the traversal **exhaustive** — remove the catch-all and
   let the compiler force every `Term` variant to be matched — not to patch in
   just the variants you happen to need. Compiler-enforced exhaustiveness
   catches the **next** such gap for free.
