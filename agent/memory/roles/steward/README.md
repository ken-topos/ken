...
| [publisher-flags-are-description-not-body-and-failure-is-silent](publisher-flags-are-description-not-body-and-failure-is-silent.md) | ★★ There is no `--body`: it dies `unknown argument` and the wrapping task STILL reports exit 0, so "completed" is compatible with nothing landing. Verify on `origin/main` BY CONTENT — never by SHA, task status, or exit code — and pick a discriminator the WP actually ADDS |
