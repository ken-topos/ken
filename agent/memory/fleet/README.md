...
| [a-green-pilot-is-not-evidence-for-a-shape-it-never-produced](a-green-pilot-is-not-evidence-for-a-shape-it-never-produced.md) | A pilot's value is its COVERAGE, not its greenness — draw the matrix of shapes the ROLLOUT will emit and mark which cells the PILOT produced; and draw the AXES on what the MECHANISM forks on, not on what the SYNTAX makes salient (an axis that perfectly separates pass from fail on a small sample is correlation, not cause) |
| [a-squash-merged-branch-cannot-be-continued](a-squash-merged-branch-cannot-be-continued.md) | Two branches must not share a name across a merge boundary — a squash-merged branch dangles AHEAD of main while its content is already in, and a publisher still parked on it HARD-BLOCKS the receiving team |
| [a-risk-register-is-a-grep-list-not-a-forecast](a-risk-register-is-a-grep-list-not-a-forecast.md) | Every entry in a report's "Risks" section is a QUESTION — `is this already true?` — not a forecast; and audit any gate (`cfg`, flag, predicate) that is WIDER than the fact it guards |
...
| [a-vacuous-law-has-zero-trust-delta](a-vacuous-law-has-zero-trust-delta.md) | A gate keyed on trust movement is BLIND to vacuity — a hollow conditional law adds no Axiom and has zero delta; ship the reaching lemma, and when you argue a gate can be skipped, name the RISK it exists to catch |
