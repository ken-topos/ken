...
1. Copy the declaration's leading provenance or delivery-stage marker
   verbatim. `DRAFT v0`, `K1 elaborated`, and `impl-ready (L2)` are not
...
3. A `DRAFT` marker does not weaken an explicitly normative contract, and an
   ...
   spelling.
4. If a file has no `> Status:` declaration, assign neither axis by inference.
   Record it in the unclassifiable report.
...
The middle column is the leading marker copied from each file's own
`> Status:` declaration. The final column applies the binding-force rule above
to that same declaration, including its qualifications.

| File | Declared provenance/stage marker | D1 binding-force assignment |
|---|---|---|
...
| `10-kernel/16-observational.md` | `K2 elaborated; K2c series-2 completes the three obs-reduction seams` | Normative for the observational interface, computation rules, and reduction behavior; `(oracle)` rules retain that qualification. |
| `10-kernel/17-conversion.md` | `K2c elaborated` | Normative for kernel conversion and the SCT gate. |
| `10-kernel/18-judgments.md` | `K-api elaborated` | Normative kernel API contract; its freeze claim is conditional on the declared landed admission gates. |
| `10-kernel/18a-primitive-registry.md` | `Phase 1 — registry audit` | Conditional native registry seal: an operation is native iff its stated correctness AC holds. |
| `10-kernel/README.md` | `K2 elaborated` | Normative WS-K contract and WS-V re-checking target. |
| `20-verification/21-spec-syntax.md` | `V1 elaborated` | Normative for verification forms, meaning, elaboration, and status model; concrete spelling is cross-referenced. |
| `20-verification/22-obligations.md` | `V2 elaborated` | Normative for obligations and their extraction algorithm. |
| `20-verification/23-prover.md` | `V3 elaborated` | Normative for the prover contract, verdicts, classifier, and certificate route; named frame axioms remain `(oracle/standard)`. |
| `20-verification/24-diagnostics.md` | `V4 elaborated` | Normative for diagnostic mechanisms and meaning; serialization belongs to `25-protocol.md`. |
| `20-verification/25-protocol.md` | `T1 elaborated` | Normative for message shape, verdict cross-walk, and stability; exact JSON names remain a finalization surface. |
...
| `30-surface/30-taxonomy.md` | `Normative` | Normative for the built-in/prelude/package boundary and minimality invariant; membership grows through its derivation process. |
...
| `30-surface/33-declarations.md` | `normative for the features` | Normative for features and the named module, visibility, and class contracts; concrete spelling remains proposal-level. |
...
| `30-surface/36-effects.md` | `L5 elaborated` | Normative for the model and elaboration; concrete spelling remains proposal-level. |
...
| `30-surface/38-ffi-io.md` | `§1.1–§1.6 impl-ready (L6); §1.7 contract-pinned (PX8-T); §2–§4 impl-ready (L7)` | Normative for trust and effect discipline with the stated per-section delivery stages. |
| `30-surface/39-elaboration.md` | `DRAFT v0` | Normative for elaboration output and guarantees; §5 is V0 implementation-rigorous and the other named sections remain frame-level. |
...
| `40-runtime/42-evaluation.md` | `X1 elaborated` | Normative reference-evaluation contract for the declared pure-core and effect-driver scope. |
...
| `40-runtime/45-native-backend.md` | `DRAFT v0` | Binding backend design for model, trust posture, and differential discipline; target choice remains open. |
| `40-runtime/46-checked-core-package.md` | `DRAFT v0` | Normative for the stable checked-core compiler input and its stated exclusions. |
| `40-runtime/47-erasure-runtime-ir.md` | `DRAFT v0` | Normative for erasure, runtime IR, loud refusal, and interpreter comparison; backend details are excluded. |
| `40-runtime/48-executable-artifact-contract.md` | `DRAFT v0` | Normative for the executable-artifact contract and loud rejection rules; emission and ABI details are excluded. |
...
| `50-stdlib/57-collections-and-views.md` | `DRAFT v0` | Binding CAT-3 contract for collection laws and views; build work remains staged as declared. |
| `50-stdlib/58-maps-sets-relations.md` | `DRAFT v0` | Binding CAT-4 contract for keyed collections, sets, and the relations frontier; build scope remains split as declared. |
| `50-stdlib/59-parsing-syntax-diagnostics.md` | `DRAFT v0` | Binding CAT-5 package contract with the stated compiler/reflection exclusions. |
| `50-stdlib/60-length-indexed-vectors.md` | `normative` | Normative for the Vec family and landed operations; `zip` and `lookup` remain gated on `DS-5c`. |
| `50-stdlib/README.md` | `DRAFT v0` | Binding standard-package-tier boundary and derivation-path requirement. |
| `60-security/61-information-flow.md` | `Sec1 + Sec1ct elaborated` | Normative for the declared IFC and constant-time discipline; surface spelling remains proposal-level. |
| `60-security/62-authority.md` | `Sec2 elaborated` | Normative for authority, attenuation, revocation contract, and audit points; spelling and named runtime mechanisms remain qualified. |
...
| `60-security/64-trust-model.md` | `Normative.` | Normative for the TCB, security reading, trusting-trust invariant, and honest limits. |
| `60-security/65-policy.md` | `Normative` | Normative for policy shape, binding guarantee, and semantics; concrete policy syntax remains deferred. |
| `60-security/README.md` | `DRAFT v0` | Normative security frame, threat model, guarantees, and limits. |
| `70-behavioral/71-assumption-boundary.md` | `impl-ready` | Normative for projection, schema, and seam discipline; literal wire spellings remain `(oracle)`-tagged. |
...
| `70-behavioral/74-agentic.md` | `impl-ready (B4)` | Normative for the agentic-assurance reduction; it adds no new agentic mechanism. |
| `70-behavioral/README.md` | `DRAFT v0` | Normative for Ken's half of the behavioral seam. |
| `90-open-decisions.md` | `living document — for the operator.` | Operator decision register; draft recommendations do not resolve the genuine forks they record. |
...
- **Different declaration shapes:** `10-kernel/11-syntax.md` keeps
  `K1 elaborated` as provenance and is normative for grammar/scoping;
  `30-surface/34-data-match.md` keeps `impl-ready (L2)` and is normative for
  its feature; `60-security/65-policy.md` has no work-package provenance
  marker and binds only the policy shape, guarantee, and semantics while
  deferring syntax. Each result follows the same two-axis rule.
