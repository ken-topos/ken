...
5. When a constructor argument contains recursive values through a declared
   strictly-positive parameter path, elaborate its methods against the lifted-IH
   parameter generated by `14 §3.2`. A nested match refines the containing value
   and its lift in lockstep, placing the motive instance for each exposed
   recursive child, and the residual lift for each exposed enclosing child, in
   the corresponding branch context. Structural recursion and proof-returning
   branch elaboration consume those entries; they do not synthesize an unchecked
   self-call or erase the lift.
