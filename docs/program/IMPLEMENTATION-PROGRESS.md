...
2026-07-25 04:55:54Z — from 73 issue file(s) in `docs/program/issues/`.
...
| `RT-FNSPLIT-B2A-C` | plan↔lowering occurrence correspondence — transport the preallocated StaticOriginId to the site where it is out of scope | active | runtime | L | none | — |
...
- `RT-FNSPLIT-B2A-S` blocked by `RT-FNSPLIT-B2A-C` (status: active)
