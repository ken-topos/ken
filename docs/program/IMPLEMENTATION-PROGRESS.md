...
2026-07-26 19:25:02Z — from 92 issue file(s) in `docs/program/issues/`.
...
| `KW-ORACLE-REMOVE` | Delete the whole-tree source-text oracle: it asserts facts about repository text, which is now a prohibited test subject | ready | language | S | none | — |
| `KW-THEOREM` | rename the surface keyword `lemma` to `theorem` | merged | language | M | none | — |
| `LIB-GATE-DECOUPLE` | main is red on two library documentation-census gates: the currency gate the operator decoupled from merges still fires from inside CI, and a doc-only merge invalidated the ledger unreported | ready | verify | S | none | — |
...
| `RT-FNSPLIT-B2E` | semantic boundary-value elimination — an opaque boundary inhabitant plus a mechanically closed operation-by-class disposition ledger over every reachable Lowered consumer, inert | ready | runtime | L | none | — |
...
| `SPEC-31-WIDTH-ERRATUM` | spec 31-lexical mandates a 96-column canonical width while the formatting conformance suite asserts 88 in 18 places and cites 31 §1d as its source — rule the exact value and reconcile | ready | spec | S | none | — |
...
| `SPEC-STORE-SPLIT` | Split durable canonical bytes from in-process maximal sharing: demote the store mechanism to private, retarget the conformance rows that assert it, and re-cut the runtime program against the relaxed contract | ready | spec-enclave | L | none | — |
...
- `KW-ORACLE-REMOVE` — Delete the whole-tree source-text oracle: it asserts facts about repository text, which is now a prohibited test subject
- `LIB-GATE-DECOUPLE` — main is red on two library documentation-census gates: the currency gate the operator decoupled from merges still fires from inside CI, and a doc-only merge invalidated the ledger unreported
...
- `SPEC-31-WIDTH-ERRATUM` — spec 31-lexical mandates a 96-column canonical width while the formatting conformance suite asserts 88 in 18 places and cites 31 §1d as its source — rule the exact value and reconcile
- `SPEC-STORE-SPLIT` — Split durable canonical bytes from in-process maximal sharing: demote the store mechanism to private, retarget the conformance rows that assert it, and re-cut the runtime program against the relaxed contract
...
- `RT-FNSPLIT-B2F` blocked by `RT-FNSPLIT-B2E` (status: ready)
