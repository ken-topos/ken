...
2026-08-05 16:07:14Z — from 163 issue file(s) in `docs/program/issues/`.
...
| `RT-UNIT-CLOSURE-CONVERT` | Activate function-unit closure conversion for predeclared units — a retained nested body's free de Bruijn references become declared typed capture slots, reconstructed at unit entry from exact caller operands | active | runtime | TBD | none | — |
...
- `RT-UNIT-CLOSURE-CONVERT` blocked by `RT-CONTSRC-PRODUCER-LOCAL` (status: active)
