...
2026-07-26 04:24:50Z — from 83 issue file(s) in `docs/program/issues/`.
...
| `KW-ORACLE-CLOSURE` | close the KW-THEOREM source oracle structurally — the occurrence sweep is never applied, and the file population is a five-arm hand enumeration | ready | language | S | none | — |
...
| `RT-FNSPLIT-B2V` | executable boundary-value ABI — one closed 64-bit tagged word for ValueWord/ResultWord plus the emitted-code interface to construct, discriminate and project it | active | runtime | L | none | — |
...
- `KW-ORACLE-CLOSURE` — close the KW-THEOREM source oracle structurally — the occurrence sweep is never applied, and the file population is a five-arm hand enumeration
