...
>      `ensures`/refinement, `law`/`prop`/`theorem`+`prove`, `Axiom`+`trusted_base()`
...
> (PR #386, `def` + `prop`/`theorem`/`proof`), proof-techniques §1 erratum (PR #387,
...
>   (→Ergo/Language), elaborator (→Ergo), reusable `def`/`theorem`/`prop` (→Core
