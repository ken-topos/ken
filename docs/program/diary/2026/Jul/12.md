...
>   `Proved`→`tt` control RED). SURF-1 fold (`fn reflTwo`→`theorem`, Ω-purity-forced)
...
>   pass — **overwhelmingly collection-interaction laws that STAY theorem**; attach
...
>   stay theorem; QA APPROVE + Architect-terminal APPROVE `evt_6wk9e45h3ykfw`,
...
>   (interaction laws STAY theorem); showcases = Librarian discretion; fidelity =
...
>   161 interaction laws stay theorem; QA+Architect-terminal APPROVE
...
> `S::name`), **not role**; `theorem` = no single owning subject. Tie-breaker: name
> exactly one owning subject → attach, else `theorem`; reuse is orthogonal (a
...
>   …); interaction laws (`mul_add_distrib_*`, `ord_nat_total`) stayed `theorem`;
...
>   `theorem` — Architect leans "non-API showcases stay `theorem`."
...
> closes #21 (fn⊥theorem partition) and completes #19.** Frame was `76d74621` (PR
> #529); code `8c65aaed`.
...
>   drift = 679 sites → `theorem`; 13 `_ind` collapses; 9 bounded `Top`; catalog `tt`
...
>   every `= Axiom` a `const`→`theorem` keyword flip, no net-new trust surface;
