...
> string_to_list_char_retraction` ×2 on main; zero `theorem…=Axiom` fossils;
...
> `docs/program/wp/axiom-fossil-sweep.md`).** Grounded: `theorem … = Axiom` →
...
> (`matches("Axiom").count()==1`,
> `contains("theorem string_to_list_char_retraction")`).
...
> `…injective` lemma stays `theorem`); capstone `+= "axiom"` only.
...
> entry (audit label incl.) as `theorem N:T=Axiom`? Architect (Sol/T1) auto-woke,
...
> `theorem` path → single kernel producer `declare_postulate`, same audit label +
...
> next Foundation catalog WP: `theorem = Axiom → axiom` (lowercase) migration in
...
> publisher FULL CI.
>
> **NEXT after kenfmt merges:** (2) fossil sweep (`theorem = Axiom → axiom`:
