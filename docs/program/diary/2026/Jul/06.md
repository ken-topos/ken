...
> initiative: #11 pedagogic catalog rewrite (def/prop/theorem/proof, top-down) —
...
> `def`/`prop`/`theorem`/`proof` per the §7.1 authoring convention (merged
...
> `theorem` form. See judgment-log D14.
...
> documents: Ω-typed laws → original-name `theorem`, computation/proof-relevant
...
> TCB, 24/24 acceptance. Validates the def/prop/theorem/proof pedagogic pattern
