...
> 2. **Fossil sweep** (`theorem = Axiom → axiom`: StringBijection + proof-techniques
...
>
> **📌 Pat fossil sweep** (`theorem = Axiom → axiom`: StringBijection +
...
> `theorem = Axiom → axiom` fossil sweep (StringBijection + proof-techniques ×2 —
> OUTSIDE P2 scope).**
>
...
> Pat's `theorem = Axiom → axiom` fossil sweep folds in — BUT fossils are OUTSIDE
...
>
> **📌 PAT DECISION (2026-07-14): the `theorem = Axiom` → `axiom` fossil migration
...
> (`theorem foo : T = f Axiom Axiom`). My frame enumerated a closed *corpus*
...
> `fn` → `theorem` fix for Ω-valued decls, a REAL currency defect and a REAL repair).
...
> repair (`fn bool_eq_sound` → `theorem`, an Ω-valued decl wrongly declared `fn`) is
