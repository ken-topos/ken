...
| the pages | **five**: `library/quickstart.md`, `library/introduction.md`, `library/learn/exercises/exercises.md`, `library/learn/exercises/README.md`, `library/learn/reading-ken/04-effects-capabilities-and-authority.md` |
| their drifted sources | **9 distinct paths / 14 citations** — ⚠ **re-derive at your base** |
...
⛔ None of these five pages is attested, so editing them adds no row.
...
**D1 — per-source claim reconciliation, all 9 sources, across all five pages.**
...
**D2 — repair what is false**, in these five pages only.

**D3 — cross-page sweep per repaired claim class**, across all five.
...
>
> **Three sources are checked catalog code** (`EmptyDec`, `Combinators`,
> `Property`) plus `catalog/guide/decomposition-abstraction.ken.md`. A claim
> about what a package provides is verifiable, ⛔ not a matter of taste.
>
...
**AC-1 — all 9 sources addressed against current blobs, for each page that
cites them**, every multi-anchor citation at each anchor. **Control:** quote
what you read. ⛔ Not from this frame, ⛔ not from memory.

**AC-2 — every claim any of these five pages makes about a drifted source is
true at your base.**

**AC-3 — cross-page closure per repaired class.** **Control:** name the class
and show the sweep across all five, ⛔ not only the edited lines.

**AC-4 — scope is exactly these five pages, drift population unchanged at 28.**
**Control:** `git diff --name-only` shows only those five paths;
...
- ⛔ **No sixth page**, and specifically ⛔ not `exercises/solutions.md`
  ([[DOC-ASBUILT-SOLUTIONS]]) and ⛔ none of the four chapters in
  [[DOC-ASBUILT-CHAPTERS]]. ⚠ `exercises.md` is in scope here; `solutions.md`
  is **not** — ⭐ if a repair to one implies a repair to the other, `D4`-route
  it and name the pair.
...
3. **A repaired class extends to a page outside these five** ⇒ ⛔ do not follow
...
all five pages**, ⛔ not back into one-WP-per-page.
