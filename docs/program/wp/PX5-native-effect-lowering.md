...
- **Depends on:** PX4 (merged) **and PX4B (base native-CLI production spine —
  `PX4B-base-native-production-spine.md`, the predecessor).** PX5 **consumes
  PX4B's public production result**; it does NOT build the producer and MUST NOT
  synthesize/feed a naked `RuntimeExpr` (Architect ruling `evt_3bsx082257v92` §4).
  The held scaffold (WIP `1ad70339`) replays **above** PX4B; any default/string-
  authored plan or naked-IR production path is removed or confined to tests.
  **Gate:** G-Sec / native-effect lane. **Sibling:** PX6 (Verify) — PX6 consumes
  what PX5 produces; both wait on PX4B landing.
