...
- **Landed packages** — `catalog/packages/Data/Collections/Derived.ken.md` (7-combinator `List`/`Nat` floor +
  string surface `concat`/`slice`/`charAt`/`eq`/`compare` + local `OrdResult`),
  `catalog/packages/Core/Classes/LawfulClasses.ken.md` (`Ord`/`Eq`/`DecEq`). Examples **import and reuse**
