...
so this is releasable. **Absorbs** the held `fn-theorem-partition` WP and
`proof-vocabulary`'s deferred `_ind`→recursive-`theorem` collapse.
...
1. **`fn`⊥Ω partition (enforce).** `fn`/`const` must **not** classify at Ω;
   `theorem`/`proof` are the only Ω-valued definitions. Makes the erasure boundary
...
`theorem mul_zero_r : IsTrue … = Proved` in a single edit).

## Settled design — FIXED inputs

Part 1 grounds on Architect ruling **evt_6eyh27x6p7h0x** (endorse; the
former `fn-theorem-partition` frame). Parts 2–3 are operator-decided
...
**(2) Zero-soundness surface (whole WP).** `fn`/`const` and `theorem`/`proof` both
...
(`elab.rs:5396`). Nothing downstream keys on `fn`-vs-`theorem`.
...
the Ω-`fn`→`theorem` migration, or it rejects the existing tree. Single branch;
...
rule ("`fn`/`const` compute; use `theorem`/`proof` for an Ω-valued definition").
...
- **Part 1:** every Ω-valued `fn`/`const` → `theorem`/`proof`. This **subsumes** the
  `_ind`+thin-wrapper idiom → single recursive `theorem` (those `_ind` fns are
  exactly the Ω-valued fns the gate rejects; A makes the recursive `theorem`
...
must not classify at Ω (`theorem`/`proof` are the only Ω-valued term defs); §16 —
...
- **Mixed `fn`↔`proof` mutual cycles** — an Ω-`fn` in a mixed cycle can't
  migrate to `theorem` until `proof-vocabulary`'s deferred mixed-cycle support
...
4. **`_ind` collapse.** The `proof-vocabulary` `_ind`+wrapper pairs become single
   recursive `theorem`s, each preserving the public law's name + signature + proof.
...
- **Gate + rename + migration:** Architect **surface + fidelity** review (gate is
  surface; Ω-`fn`→`theorem` and `tt`→`Proved` are behaviour-preserving relabels/
...
`fn` provably means "computes", `theorem`/`proof` mean "proof-irrelevant" (enforced
