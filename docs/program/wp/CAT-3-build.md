...
2. **The `view` family name remains the operator-chosen concept name.** Use
   ...
   `project`, per `57 §4.2`.
3. **CAT-1-build is landed.** `packages/lawful-functors/lawful_functors.ken`
...
4. **CAT-4 is landed but not a dependency.** `packages/collections/map.ken`
...
In `packages/collections/collections.ken`, verify which operations are already
