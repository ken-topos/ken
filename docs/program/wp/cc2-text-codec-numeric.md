# WP CC2 — `Data.Text.Codec` + `Capability.Parsing.Numeric` (+ gated lawful `String` keys)
...
0. **Landed substrate (grounded @ 6088e0b8 — DO NOT rebuild).**
   - `Char := {c:Int | isScalar c}`; `charToInt c = c` is a **landed identity
     ...
     written as scalar `Int`s, e.g. `(48:Int)` (per `Text.ken.md:18` idiom
     `Cons Char (10:Int)`).
...
1. **Lawful `DecEq`/`Ord String` = transport through `List Char`; ONE named
   ...
   - **GATING (critical):** lawful `DecEq`/`Ord String` is the **FINAL, GATED**
     ...
     `Data.Text.Codec` + `Capability.Parsing.Numeric` **proceed regardless**.
...
2. **Located errors = a minimal in-package located carrier, NOT generic-over-`e`.**
   - Do **NOT** overload `Utf8Error = InvalidUtf8` (nullary, decode-specific) for
     numeric parse failure. `Capability.Parsing.Numeric` (or a tiny `Text` error module) owns a
...
3. **Digit extraction = landed `charToInt` + `leq_int`, NO new primitive.**
   ...
   `char_to_int` / `to_digit` / `ord` primitives. `Capability.Parsing.Numeric` folds
...
4. **Bijection-debt confinement (Architect guard-rail `evt_49zk7vabtbfq8`) —
   ...
   gated keys cert (fixed input 1); do **NOT** let it leak into `Capability.Parsing.Numeric` /
   `Data.Text.Codec` verified laws. Grounded @ 6088e0b8: **neither** bijection
   ...
   - **(a)** `Data.Text.Codec` + `Capability.Parsing.Numeric` ship parse / format / view as
   ...
   - **(c)** `Data.Text.Codec`'s round-trip rides the **landed** `BytesRoundTripLaw`
     (`decode ∘ encode = Ok`, `Bytes↔String`; reverse explicitly not-a-law) — do
     **NOT** assert a `String↔List Char` crossing view law.
   `Capability.Parsing.Numeric` is confirmed **independent of** `DecEq`/`Ord String` (parse /
...
1. **`Data.Text.Codec`** — ASCII/UTF-8 views over `bytes_decode` / `bytes_at`. Define
...
2. **`Capability.Parsing.Numeric`** — `char_to_digit` (fixed input 3); `parse_nat` / `parse_int`
...
- **AC1 — DS-7/8 ordered shared-`ElabEnv` acceptance harness.** Following the
  ...
  picks (i)]** → Data.Text.Codec → Capability.Parsing.Numeric — then the CC2 entries **including
...
- **AC4 — `Capability.Parsing.Numeric` parse/format: located discriminators + structural
  round-trip.**
...
- **AC5 — `Data.Text.Codec` elaborates and classifies.** `decode_utf8` wraps
...
- **AC6 — lawful `String` keys (GATED).** IFF the operator picks option (i): the
  ...
  Data.Text.Codec/Capability.Parsing.Numeric close.
- **AC7 — conformance seed for `Capability.Parsing.Numeric` parse semantics (→ CV vote).** A
...
- **AC8 — scope discipline.** Only the new `Data.Text.Codec` / `Capability.Parsing.Numeric` catalog
...
- `Data.Text.Codec` / `Capability.Parsing.Numeric` are **ordinary catalog Ken with zero
  kernel/prelude delta** — no kernel rule, no prelude emission.
- **No `String`-crossing round-trip law in ANY `Data.Text.Codec`/`Capability.Parsing.Numeric`
...
`Data.Text.Codec` and `Capability.Parsing.Numeric` proceed **immediately** (they do not depend on
...
Data.Text.Codec + Capability.Parsing.Numeric land and retros are in; the gated `String`-keys
