# catalog-tree-migration — move `packages/` under `catalog/`
...
Introduce a top-level `catalog/` directory and move `packages/` to
`catalog/packages/` verbatim (`git mv`, no internal renames), updating every
...
catalog/
  ...
  packages/             light container: a README + one subdir per package
    README.md           package index / navigation (today's packages/README.md,
...
- **Verbatim move.** `git mv packages/<dir> catalog/packages/<dir>` for each
  of the six package dirs (`collections`, `lawful-classes`,
  `lawful-functors`, `parsing`, `transport`, `verify`) and for
  `packages/README.md` → `catalog/packages/README.md`. No content edits
...
- **Three new stub files**, each a heading + one paragraph + pointers to `06`
  ...
  - `catalog/packages/README.md` does **not** need to be authored fresh — it
    is today's `packages/README.md`, moved verbatim (see above). Do not
    duplicate it.
...
Snapshot taken at authoring time (`git grep -n 'packages/'`, repo root, HEAD).
...
Total: **303** occurrences of the literal string `packages/` across the
tracked tree (`git grep -n 'packages/' | wc -l`), zero of which are in
`*.toml`, `scripts/**`, or `.github/**` (all zero — confirmed by grep; the
build/CI config has no `packages/` literal to update).
...
| Conformance seeds | `conformance/**` | 15 | 38 | prose citations (`` `packages/...` ``) inside seed `.md` files, plus 4 `.ken` challenge fixtures each citing one package path in a comment |
...
| Docs — `docs/program/wp/` | `docs/program/wp/*.md` | 28 | — | historical + open WP frames citing `packages/...` paths |
...
| Docs — `packages/` (moving with the tree) | `packages/**/*.md` | 5 | — | `packages/README.md` + four `MANIFEST.md` files — these move as part of the `git mv`, not a separate rewrite pass (only rewrite if a `MANIFEST.md` cites another package's repo-relative path) |
...
cite `packages/` more than once); Rust and conformance rows give raw
occurrence counts.

**Rust test-fixture breakdown** (`git grep -c 'packages/' -- crates/**/*.rs`,
test files only):
...
`packages/` → `catalog/packages/` string substitution inside the literal:

- `include_str!("../../../packages/<pkg>/<file>.ken")` — nine files, compile-
  time embed relative to the test file's own crate dir. **Prefix depth is
  unchanged** (`catalog/packages/` sits at the same tree depth as
  `packages/` — only the path *string* changes, not the number of `../`
  segments).
- `workspace_root().join("packages/<pkg>/<file>.ken")` — `rosetta.rs` only,
...
test) embeds `packages/verify/proof_erasure_boundary_checker.ken` via
`include_str!` at line 15 and cites the same path in an error string at line
69. This is the **only** `packages/` reference in a crate that ships in the
...
1. **The move.** `git mv` each of the six package dirs and `packages/
   README.md` into `catalog/packages/`. **AC:** `git status` shows renames
   (not add+delete) for every file under the old `packages/` tree; `ls
   packages/` fails (dir gone); `catalog/packages/` matches the target
   layout in §1.
...
3. **Production src.** Rewrite the two references in
   ...
   checker still pass; `git grep -n 'packages/' crates/ken-interp/src`
...
5. **Conformance seeds.** Rewrite the 38 prose-citation occurrences across
   ...
   fixtures). **AC:** no seed cites a `packages/`-rooted path; the seeds'
   own pass/fail status is unchanged (this is a prose-only rewrite — no
   seed's expected behavior changes).
6. **Docs.** Rewrite `packages/`-rooted path references across the 70
   markdown files enumerated in §3 (`docs/program/` incl. `wp/`, `spec/`,
   `agent/`, the four `packages/*/MANIFEST.md` files if they cross-cite, and
   ...
   `git grep -rn 'packages/' -- '*.md'` returns zero hits outside
...
7. **Global no-regression AC.** `scripts/ken-cargo test --workspace` green.
   ...
   grep -n 'packages/' | grep -v '^catalog/'` returns **zero** hits (the
...
- Do **not** author real content into `catalog/README.md`,
  `catalog/REFERENCES.md`, or `catalog/packages/README.md` beyond the stub
