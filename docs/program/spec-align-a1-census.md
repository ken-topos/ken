...
|---|---|---|---|---|---|---|
| Exact in-process FNV-1a addressing, full-byte `memcmp`, monotonic `u64` slots | `41 §3,§3b`; `44 §1,§2` | 4 for algorithm/allocation; 1 for no false merge; current-code 2 coupling | `runtime/capacity/no-lattice-on-hot-path`; `runtime/capacity/reset-retires-ids-never-resurrected` | `ken-runtime/src/{hash,store}.rs`; `ken-foundation/src/{hash,store}.rs`; the exported runtime hash is also reused by artifact/package modules | The spec separates durable hashes, but current artifact/package code reuses `fnv1a_64`; function-level relaxation is not isolated | **STOP-4/1/2** — live rows, C7/B2E entanglement, and current durable-consumer coupling |
...
| Same-slot dedup, slot-observable structural sharing, O(1) equality | `41 §2,§4`; `42 §3.4`; `44 §1` | 1/4, equality/cost/physical-sharing axes | `runtime/seed-runtime.md`; `runtime/values/{dedup-shares-slot,equality-is-slot-id}`; `runtime/evaluation/seed-evaluation.md` sharing rows; `surface/collections/seed-collections.md` sharing rows | store interning and interpreter slot-bearing canonical values | Surface equality/cost and conformance-observed sharing | **STOP-1/4** — live population and C7 |
...
| Bignum canonical tag `0x01` | `35 §2.1`; `41 §3a` | 2, durable canonical-byte profile | `surface/numbers/seed-numbers.md` AC1; `surface/numbers/seed-f1-bignum-int.md` AC1/AC3 | both canonical encoders fix `BIG_INT = 0x01`; boundary/native integer paths consume it | Canonical bytes, publication and artifact identities | **STOP-2** — versioned-profile/threat work belongs to Track B |
| Sign-magnitude, `u64` limbs, minimal-limb/one-zero-limb canonicalization | `35 §2.1`; `41 §3a` | 2 for durable encoding; 1 for unique numeric value | `runtime/values/bignum-minimal-limb-encoding`; `surface/numbers/seed-f1-bignum-int.md` AC3; `conformance/README.md` F1 row | `ken-runtime/src/canonical.rs`, `native_int.rs`; Foundation canonical encoder | Canonical byte consumers and reproducible identities | **STOP-2/1** — minimality is the uniqueness contract, not a free limb-layout choice |
| `Decimal` inline `{i64 coeff, i32 exp}` fast path and heap tag `0x0A` | `35 §2,§2.3`; `41 §3a,§5`, contradicted by normative `18a §5.6.1` | 4 for runtime cutoff/layout; 1 for coefficient/exponent semantics; 2 for tag | `surface/numbers/seed-numbers.md` repeats the old lowering; `runtime/values/immediate-vs-interned-boundary` asserts the small/immediate split; `surface/numbers/demote-removes-decimal-char-primitives` instead requires derived `(coeff : Int, exp : Int)` and removal of primitive Decimal representation | runtime/Foundation still define `SmallDecimal` and `BIG_DECIMAL`; interpreter Decimal is constructor-derived, so occurrence cannot select authority | Durable canonical bytes for the tag; none found for the inline struct | **STOP-1/2/4** — live normative/conformance contradiction across type/producer, coefficient carrier, tag, and fast-path axes; separately tracked reconcile required |
...
| Exactly two ASCII spaces per indentation level | `31 §1d` | 4, layout policy | `surface/formatting/breakable-syntax-never-exceeds-88-columns`; formatter indentation cases; multi-binding-let byte goldens | `ken-elaborator/src/layout.rs` fixes `INDENT_WIDTH = 2` | Canonical source bytes | **STOP-4** — multiple live exact-byte consumers |
| Four literate fence roles and exact formatter splicing/exemption policy | `31 §1d` | 1 for role meaning; 4 for formatting/splicing mechanics | formatter FMT1/FMT4/FMT5 and `four-fence-roles-and-narrow-exemption` | `ken-elaborator/src/literate.rs` and `format.rs` | `.ken.md` producers/readers and catalog validation | **STOP-1/4** — role semantics and private formatter policy are not separable inside the live rows |
...
```
$ rg -n '4 MiB' conformance
conformance/runtime/capacity/seed-capacity.md:179:
