<!-- This PR is opened by the STEWARD (the sole publisher) from a team's local
     wp/<ID> branch, to run CI. Review happens in mootup (the merge Decision), not
     here — see docs/program/04-git-and-integration.md. One work package per PR;
     small merges keep `main` green. -->
<!-- Authoring agents never open PRs; this template is the Steward's checklist. -->

## Work package
<!-- WP ID from docs/program/03-program-of-work.md, e.g. K1, V3, L2 -->
WP:

## What this does
<!-- One or two sentences. -->

## Acceptance criteria satisfied
<!-- Quote the WP's acceptance criteria and note how each is met. -->
- [ ]

## Spec source (clean room)
<!-- Cite /spec and /conformance entries this implements. Do NOT cite copyleft source. -->
<!-- Confirm: no AGPLv3 or other copyleft source was copied or closely paraphrased. -->
- [ ] Implemented from spec/conformance, not from copyleft source.

## Conformance
<!-- The pre-merge gate: build + conformance + clean-room must be green. -->
- [ ] Build + conformance suite green in CI.

## Cross-team impact
<!-- Does this change an interface other teams depend on? Name the teams; the Steward notifies them on merge. -->

---
<!-- Merge gate (Steward): merge Decision approved (Architect always + Spec on its paths) AND CI green AND clean-room green AND no gate regression. -->

