...
//! post-fix flip.
//!
//! Every case asserts two independent discriminators:
//!
...
//!
//! Carrying both axes is what makes every case discriminating. On the variant
//! axis alone the `u64::MAX`-sentinel single-fault cases are green-vs-green:
...
//! | `buffer_freeze_narrowing_is_unreachable...` | passes | none -- structural premise, deliberately fix-independent |
//!
//! `BufferFreeze` has no case here by structural necessity, not by omission --
//! see `buffer_freeze_narrowing_is_unreachable_from_checked_source`.
...
/// `BufferFreeze` has **no** executable narrowing case at this level, and the
/// reason is structural rather than an omission (frame AC-5).
///
/// `freeze`/`spanBytes` derive both host-width arguments from a `BufferSpan`
/// (`buffer_span_start` / `buffer_span_length`), and the prelude removes
/// `PrivateBufferSpan` from the public global scope. Every `BufferSpan`
/// reaching `freeze` is therefore host-minted by a successful `readAt`, so its
/// start and budget are already in range: checked source cannot present a
/// malformed argument to this consumer at all, with or without a coincident
/// resource fault.
///
/// The assertion is a discriminating pair rather than a lone rejection, so it
/// cannot pass for the wrong reason: the public `MkBufferWindow` control -- the
/// same shape, a constructor applied to two arguments -- must elaborate while
/// the private span constructor must not.
///
/// This pins the reachability premise so the honest "not covered here" report
/// cannot go stale silently: if `PrivateBufferSpan` ever becomes constructible
/// from source, this fails and the differential owes two more cases. The
/// narrowing itself stays covered at the dispatch boundary by
/// `eval::px5b_effect_observation_tests::`
/// `rt_parity_host_width_consumers_return_exact_narrowing_errors`.
#[test]
fn buffer_freeze_narrowing_is_unreachable_from_checked_source() {
    ...
    let error = elaborates("PrivateBufferSpan (sub_int 0 1) Zero", "BufferSpan")
        .err()
        .expect("a source-forged BufferSpan must not elaborate");
    assert!(
        matches!(error, ken_cli::RunError::Elaboration(_)),
        "PrivateBufferSpan must be absent from source scope, so a malformed \
         BufferFreeze argument is unconstructible; got {error:?}"
    );
}
