...
pub(in crate::cranelift_backend) fn compile_expr_into_module<'a, M: Module>(
    ...
    while let Some(item) = compiler.partition_queue.pop_back() {
        ...
        compiler.active_partition_caller_role = match &item {
            ...
            PartitionWorkItem::SourceArm(item) => PartitionPendingCallerRole::State {
                state_id: item.state_id,
            },
            PartitionWorkItem::SourceKont(item) => PartitionPendingCallerRole::State {
                state_id: item.state_id,
            },
            PartitionWorkItem::ProducerKont(item) => PartitionPendingCallerRole::State {
                state_id: item.state_id,
            },
            PartitionWorkItem::CleanupStep(item) => PartitionPendingCallerRole::State {
                state_id: item.state_id,
            },
        };
...
impl<'a> Lowering<'a> {
    ...
    fn push_partition_source_prefix(
        ...
        let mut exit_stack = control.pending_partition_exit_stack.take();
        ...
        control.pending_partition_exit_stack = exit_stack;
        Ok(())
    }
    ...
    fn append_partition_exit_stack_values(
        stack: &Option<RecursorUnwindStack>,
    ...
    fn rebuild_partition_exit_stack_values(
        stack: &mut Option<RecursorUnwindStack>,
        ...
        let next = |values: &mut dyn Iterator<Item = cranelift_codegen::ir::Value>| {
            values.next().ok_or_else(|| {
                unsupported(
                    ...
                    "partition exit stack input cell ended early",
                )
            })
        };
        if let Some(stack) = stack {
            if let Some(cursor) = &mut stack.partition_cursor {
                cursor.capture_pointer = next(values)?;
            }
        ...
        if std::env::var_os("KEN_NATIVE_EXIT_TRACE").is_some() {
                ...
                .open("/tmp/ken-native-exit-trace")
            {
                let kind = match definition.current {
                    SourcePrefixTemplate::ReturnFromSelectedCase { .. } => "return",
                    SourcePrefixTemplate::UnwindRecursorSegment { .. } => "unwind",
                    _ => "other",
                };
                let _ = writeln!(trace, "pop node={} kind={kind}", cursor.node.0);
            }
        }
        let pointer_type = builder.func.dfg.value_type(cursor.capture_pointer);
        let successor_pointer =
            builder
                .ins()
                .load(pointer_type, MemFlags::trusted(), cursor.capture_pointer, 0);
        control.consumed_partition_cursor = Some(cursor);
    ...
    ) -> Result<Option<(PoppedPartitionRecursorLayer, PartitionSourceResume)>, CraneliftBackendError>
    {
        ...
        let source_cursor = match control.partition_cursor {
            Some(cursor) => PartitionSourceResumeCursor::Kont(cursor),
            None => PartitionSourceResumeCursor::Terminal,
        };
        control.consumed_partition_cursor = None;
        // ExitScopeComplete crosses only into this exact residual SourceKont.
        // That SourceKont, not the Complete action, restores the prior producer
        // tail after the bracket body has settled.
        let producer = PartitionProducerResumeCursor::Producer(parent_producer);
        let pending_exit_stack = control.pending_partition_exit_stack.take();
        let site = PartitionSourceResumeSite {
            target: match source_cursor {
                PartitionSourceResumeCursor::Kont(cursor) => {
                    PartitionSourceResumeTarget::Kont(cursor.node)
                }
                PartitionSourceResumeCursor::Terminal => PartitionSourceResumeTarget::Terminal,
            },
            parent_return: parent_return.node,
        ...
        Ok(Some((
            popped,
            PartitionSourceResume {
                site,
                cursor: source_cursor,
                parent_return_capture_pointer: parent_return.capture_pointer,
                producer,
                pending_exit_stack,
            },
        )))
    ...
    fn lower_computational_host_result(
        ...
        for (block, constructor, payload) in [
            ...
            let lowered = if let Some(producer_tail) = producer_tail {
                self.transfer_partition_producer_tail_before_scalarization(
                    builder,
                    lowered,
                    producer_tail,
                )?
        ...
        let lowered = if exit_merge == Some(true) {
        ...
        if let Some(tail) = producer_tail {
            let fanout_site_id = self.partition_next_site;
            self.partition_next_site =
                self.partition_next_site.checked_add(1).ok_or_else(|| {
                    unsupported(
                        "NativeProducerContinuationStepV1",
                        "inline producer-tail fanout identity exhausted",
                    )
                })?;
    ...
    fn record_pending_partition_call_edge(
        ...
        self.pending_partition_call_edges
            .push(PartitionPendingCallEdge {
                ...
                result_payload,
                local_tag_output,
                target: PartitionPendingCallTarget::Ordinary,
            });
    }

    #[allow(clippy::too_many_arguments)]
    fn defer_or_consume_partition_arm_return(
        &mut self,
        authority: PartitionBranchReturnAuthority,
        ...
        let candidates = self
            ...
            .filter_map(|(index, edge)| {
                (edge.function_index == self.active_partition_function_index
                    && edge.caller
                        == (PartitionPendingCallerRole::Arm {
                            helper_index: caller_helper_index,
                        })
                    && edge.kind == PartitionPendingCallKind::ProducerKont
                    && edge.result_payload == result_payload
                    && matches!(edge.target, PartitionPendingCallTarget::Ordinary))
                .then_some(index)
            })
            .collect::<Vec<_>>();
        let Some(&edge_index) = candidates.first() else {
            return self.consume_partition_branch_return(
                authority,
                caller_helper_index,
                actual_kind,
            );
        ...
        if candidates.len() != 1 {
            return Err(unsupported(
                "NativeFunctionPartition",
                "one Arm final scalar result aliases multiple pending producer calls",
            ));
        }
        if authority.descriptor != outer_call.branch_return
            || authority.descriptor.helper_index != caller_helper_index
            || authority.descriptor.required_kind != actual_kind
            || actual_kind != ScalarMergeKind::ExitCode
        {
            return Err(unsupported(
                "NativeFunctionPartition",
                "deferred Arm final scalar authority disagrees with its exact call edge",
            ));
        }
        let edge = self
            .pending_partition_call_edges
            .get_mut(edge_index)
            .expect("pending Arm edge index was just resolved");
        edge.target = PartitionPendingCallTarget::FinalScalarArm {
            checked_join,
            required_kind: actual_kind,
            branch_return: authority,
            caller_helper_index,
            caller_tag_pointer,
            outer_call,
        };
    ...
    fn function_value_uses(function: &cranelift_codegen::ir::Function, value: Value) -> Vec<Inst> {
        function
            .layout
            .blocks()
            .flat_map(|block| function.layout.block_insts(block))
            .filter(|inst| function.dfg.inst_values(*inst).any(|arg| arg == value))
            .collect()
    }

    fn audit_partition_arm_outer_payload_only(
        functions: &[(FuncId, cranelift_codegen::Context)],
        witness: PartitionArmOuterCallWitness,
    ) -> Result<(), CraneliftBackendError> {
        let function = &functions
            .get(witness.function_index)
            .ok_or_else(|| {
                unsupported(
                    "NativeFunctionPartition",
                    "deferred Arm target lost its outer fanout function",
                )
            })?
            .1
            .func;
        let call_block = function
            .layout
            .inst_block(witness.call_inst)
            .ok_or_else(|| {
                unsupported(
                    "NativeFunctionPartition",
                    "deferred Arm target lost its outer fanout call",
                )
            })?;
        let results_exact =
            function.dfg.inst_results(witness.call_inst) == [witness.result_payload];
        let tag_arg_exact =
            function.dfg.inst_args(witness.call_inst).last().copied() == Some(witness.tag_pointer);
        let tag_uses = Self::function_value_uses(function, witness.tag_pointer);
        let result_uses = Self::function_value_uses(function, witness.result_payload);
        let forward_opcode = function.dfg.insts[witness.forward_inst].opcode();
        let forward_args = function
            .dfg
            .inst_values(witness.forward_inst)
            .collect::<Vec<_>>();
        let suffix = function
            .layout
            .block_insts(call_block)
            .skip_while(|inst| *inst != witness.call_inst)
            .skip(1)
            .collect::<Vec<_>>();
        if !results_exact
            || !tag_arg_exact
            || tag_uses != [witness.call_inst]
            || result_uses != [witness.forward_inst]
            || forward_opcode != Opcode::Jump
            || forward_args != [witness.result_payload]
            || suffix != [witness.forward_inst]
        {
            return Err(unsupported(
                "NativeFunctionPartition",
                format!(
                    "FinalScalarArm outer fanout does not consume only the helper payload: \
                     results_exact={results_exact}, tag_arg_exact={tag_arg_exact}, \
                     tag_uses={tag_uses:?}, result_uses={result_uses:?}, \
                     forward_opcode={forward_opcode}, forward_args={forward_args:?}, \
                     suffix={suffix:?}",
                ),
            ));
        }
        let tag_loads = function
            .layout
            .blocks()
            .flat_map(|block| function.layout.block_insts(block))
            .filter(|inst| {
                matches!(
                    ...
                    InstructionData::StackLoad {
                        ...
                        if stack_slot == witness.tag_output
                )
            })
            .collect::<Vec<_>>();
        if !tag_loads.is_empty() {
            let tag_loads = tag_loads
                .iter()
            ...
            return Err(unsupported(
                "NativeFunctionPartition",
                format!(
                    "FinalScalarArm outer fanout observes the helper tag result at {tag_loads:?}",
                ),
            ));
        }
        Ok(())
    ...
    fn audit_and_rewrite_partition_arm_final_suffix(
        ...
        let call_block = function.layout.inst_block(call_inst).ok_or_else(|| {
            unsupported(
                "NativeFunctionPartition",
                "FinalScalarArm lost its pending helper call",
            )
        })?;
        let suffix = function
            .layout
            ...
            .skip(1)
            .collect::<Vec<_>>();
        let [tag_load, zero, tag_store, ret] = suffix.as_slice() else {
            return Err(unsupported(
                "NativeFunctionPartition",
                "FinalScalarArm has semantic or noncanonical work after its helper call",
            ));
        };
        ...
        let load_result_is_dead = function
            .dfg
            .inst_results(*tag_load)
            .first()
            .is_some_and(|value| Self::function_value_uses(function, *value).is_empty());
        if !load_is_exact
            || !zero_is_exact
            || !store_is_exact
            || !return_is_exact
            || !load_result_is_dead
        {
            return Err(unsupported(
                "NativeFunctionPartition",
                "FinalScalarArm post-call suffix is not dead-tag-load plus canonical scalar return",
            ));
        }
        if rewrite {
            ...
            cursor
                .ins()
                .store(MemFlags::trusted(), zero, caller_tag_pointer, 0);
    ...
    fn seal_partition_call_edges(
        ...
        for edge in edges {
            ...
            let contract = self
                .partition_continuations
                .return_contract(edge.callee_state_id)?;
                ...
                (
                    ...
                    match edge.target {
                        PartitionPendingCallTarget::FinalScalarArm {
                            ...
                            if contract.checked_join != checked_join
                                ...
                                || branch_return.descriptor != outer_call.branch_return
                            {
                                return Err(unsupported(
                                    "NativeFunctionPartition",
                                    "abrupt FinalScalarArm target disagrees with its exact contract",
                                ));
                            }
                            Self::audit_partition_arm_outer_payload_only(functions, outer_call)?;
                            let function = &mut functions
                        ...
                        PartitionPendingCallTarget::Ordinary => {
                            ...
                            let mut cursor =
                                FuncCursor::new(&mut function.func).after_inst(edge.call_inst);
                            while cursor.current_inst().is_some() {
                                cursor.remove_inst();
                            }
    ...
    fn complete_partition_producer_tail(
        lowered: Lowered,
        tail: PartitionProducerKontCursor,
        fanout_site_id: u64,
    ) -> Result<Lowered, CraneliftBackendError> {
        match lowered {
            Lowered::ProcessExitStatus { value } => Ok(Lowered::CompletedProducerTail {
                ...
                completion: PartitionProducerTailCompletion {
                    tail_site_id: tail.site_id,
                    fanout_site_id,
                },
            }),
            Lowered::CompletedProducerTail { .. } => Err(unsupported(
                "NativeProducerContinuationStepV1",
                "producer-tail completion was replayed by an ancestor fanout",
            )),
            _ => Err(unsupported(
                "NativeProducerContinuationStepV1",
                "producer-tail fanout completed without an ExitCode result",
            )),
        }
    }
    ...
    fn partition_return_tail_cursor(
        ...
        self.active_partition_return_contract
            ...
            .map_or(current, |site_id| PartitionProducerKontCursor {
                site_id,
                capture_pointer: current.capture_pointer,
            })
    }

    fn consume_partition_producer_tail_completion(
        lowered: Lowered,
        expected: PartitionProducerKontCursor,
        context: &'static str,
    ) -> Result<(Lowered, bool), CraneliftBackendError> {
        match lowered {
            Lowered::CompletedProducerTail { value, completion } => {
                if completion.tail_site_id != expected.site_id {
                    return Err(unsupported(
                        "NativeProducerContinuationStepV1",
                        format!(
                ...
                Ok((Lowered::ProcessExitStatus { value }, true))
            }
            lowered => Ok((lowered, false)),
        }
    }

    fn reconcile_partition_source_helper_exit(
        &mut self,
        lowered: Lowered,
        ...
        match (contract.live_producer_tail, lowered) {
            ...
            (Some(_), lowered) => Err(unsupported(
                "NativeProducerContinuationStepV1",
                format!(
                    "private Source helper reached scalar return with {} while exact tail {:?} \
                     remained live in {state_kind} state {state_id}",
                    lowered_value_kind(&lowered),
                    contract.live_producer_tail,
                ),
            )),
            (None, Lowered::CompletedProducerTail { .. }) => Err(unsupported(
                "NativeProducerContinuationStepV1",
                "private Source helper returned completion outside an empty-tail contract",
            )),
            (None, lowered) => Ok(lowered),
        }
    }

    fn transfer_partition_producer_tail_before_scalarization(
        &mut self,
        builder: &mut FunctionBuilder<'_>,
        lowered: Lowered,
        tail: PartitionProducerKontCursor,
        ...
        let expected_tail = self
            ...
            .as_ref()
            .and_then(|contract| contract.live_producer_tail)
            .map_or(tail, |site_id| PartitionProducerKontCursor {
                site_id,
        ...
        match lowered {
            ...
            Lowered::ProcessExitStatus { .. } => Err(unsupported(
                "NativeProducerContinuationStepV1",
                "fanout scalarized before discharging its exact producer tail",
            )),
            lowered => {
                ...
                if !consumed {
                    return Err(unsupported(
                        "NativeProducerContinuationStepV1",
                        "producer-tail helper returned without exact completion evidence",
                    ));
                }
                Ok(lowered)
            }
    ...
    fn outline_computational_host_result_arms(
        ...
        for (arm_index, block, constructor, payload) in [
            ...
            let forward_inst = builder.ins().jump(merge, &[status.into()]);
            let outer_call = PartitionArmOuterCallWitness {
                function_index: self.active_partition_function_index,
                call_inst: call,
                forward_inst,
                result_payload: status,
                tag_output,
                tag_pointer: tag_output_pointer,
                branch_return: return_authority.descriptor,
            };
        ...
        let lowered = Lowered::ProcessExitStatus {
            value: builder.block_params(merge)[0],
        };
        if let Some(tail) = self.active_partition_producer_kont {
            Self::complete_partition_producer_tail(
                lowered,
                self.partition_return_tail_cursor(tail),
                partition_site_id,
    ...
    fn lower_computational_producer_expr(
        ...
        match scrutinee {
                    ...
                    let fanout_site_id = if producer_tail.is_some() {
                        ...
                        self.partition_next_site =
                            self.partition_next_site.checked_add(1).ok_or_else(|| {
                                unsupported(
                                    "NativeProducerContinuationStepV1",
                                    "Bool producer-tail fanout identity exhausted",
                                )
                            })?;
                        Some(site_id)
                    } else {
            ...
            RuntimeExpr::If {
                ...
                let fanout_site_id = if producer_tail.is_some() {
                    ...
                    self.partition_next_site =
                        self.partition_next_site.checked_add(1).ok_or_else(|| {
                            unsupported(
                                "NativeProducerContinuationStepV1",
                                "If producer-tail fanout identity exhausted",
                            )
                        })?;
                    Some(site_id)
                } else {
                ...
                if let (Some(tail), Some(site_id)) = (producer_tail, fanout_site_id) {
                    Self::complete_partition_producer_tail(
                        lowered,
                        self.partition_return_tail_cursor(tail),
                        site_id,
    ...
    fn push_scope_body_return_producer_kont(
        &mut self,
        builder: &mut FunctionBuilder<'_>,
        popped: &PoppedPartitionRecursorLayer,
        source_successor: &PartitionSourceResume,
        start: PartitionProducerKontCursor,
        checked_join: PartitionCheckedJoinIdentity,
        return_kind: ScalarMergeKind,
    ) -> Result<PartitionProducerKontCursor, CraneliftBackendError> {
        let target = popped.target.ok_or_else(|| {
            unsupported(
                "NativeExitScopeTransitionV1",
                "scope body return lost its exact recursor cell",
            )
        })?;
        let obligation = popped.exit_obligation.ok_or_else(|| {
            unsupported(
                "NativeExitScopeTransitionV1",
                "scope body return lost its exact open obligation",
            )
        })?;
        let invocation = self
            .invocation_pointer
            .expect("scope body return planning owns an invocation pointer");
        let pointer_type = builder.func.dfg.value_type(invocation);
        let null = builder.ins().iconst(pointer_type, 0);
        let source_pointer = match source_successor.cursor {
            PartitionSourceResumeCursor::Kont(cursor) => cursor.capture_pointer,
            PartitionSourceResumeCursor::Terminal => null,
        };
        let parent_producer_pointer = match source_successor.producer {
            PartitionProducerResumeCursor::Producer(cursor) => cursor.capture_pointer,
            PartitionProducerResumeCursor::Terminal => {
                return Err(unsupported(
                    "NativeExitScopeTransitionV1",
                    "scope body return has no parent producer tail",
                ));
            }
        };
        let fields = [
            obligation.capture_pointer,
            start.capture_pointer,
            source_pointer,
            source_successor.parent_return_capture_pointer,
            parent_producer_pointer,
        ];
        let frame = builder.create_sized_stack_slot(StackSlotData::new(
            StackSlotKind::ExplicitSlot,
            partition_frame_size(fields.len())?,
            3,
        ));
        self.partition_metrics
            .record_cleanup_call_frame(fields.len());
        for (index, value) in fields.into_iter().enumerate() {
            let offset = index
                ...
                .ok_or_else(|| {
                    unsupported(
                        "NativeExitScopeTransitionV1",
                        "scope body return cell offset overflowed",
                    )
                })?;
            builder.ins().stack_store(value, frame, offset);
        }
        let fresh_capture_pointer = builder.ins().stack_addr(pointer_type, frame, 0);
        let bound_capture_pointer = builder.ins().load(
            pointer_type,
            MemFlags::trusted(),
            obligation.capture_pointer,
            (PARTITION_FRAME_FIELD_BYTES * 4) as i32,
        );
        let unbound = builder.ins().icmp_imm(
            cranelift_codegen::ir::condcodes::IntCC::Equal,
            bound_capture_pointer,
            0,
        );
        let capture_pointer =
            builder
                .ins()
                .select(unbound, fresh_capture_pointer, bound_capture_pointer);
        builder.ins().store(
            MemFlags::trusted(),
            capture_pointer,
            obligation.capture_pointer,
        ...
        let site_id = self.intern_producer_kont_site(PartitionProducerKontSitePlan {
            action: ProducerKontAction::ScopeBodyReturn {
                target: target.node,
                obligation: obligation.node,
                source_successor: source_successor.site,
            },
    ...
    fn push_exit_scope_producer_kont(
        ...
        let source_successor_pointer = match source_successor.cursor {
            PartitionSourceResumeCursor::Kont(cursor) => cursor.capture_pointer,
            PartitionSourceResumeCursor::Terminal => null,
        };
        let producer_successor_pointer = match source_successor.producer {
            PartitionProducerResumeCursor::Producer(cursor) => cursor.capture_pointer,
            PartitionProducerResumeCursor::Terminal => null,
        };
        let pending_exit_pointer = source_successor
            .pending_exit_stack
            .as_ref()
            .and_then(|stack| stack.partition_cursor)
            .map_or(null, |cursor| cursor.capture_pointer);
        let pending_qualification_pointer = source_successor
            .pending_exit_stack
            .as_ref()
            .and_then(|stack| stack.partition_qualification)
            .map_or(null, |cursor| cursor.capture_pointer);
        let pending_obligation_pointer = source_successor
            .pending_exit_stack
            .as_ref()
            .and_then(|stack| stack.partition_open_obligation)
            .map_or(null, |cursor| cursor.capture_pointer);
        ...
        let complete_fields = [
            ...
            source_successor_pointer,
            source_successor.parent_return_capture_pointer,
            producer_successor_pointer,
            pending_exit_pointer,
            pending_qualification_pointer,
            pending_obligation_pointer,
        ];
    ...
    fn install_source_recursor_invocation<'b>(
        ...
        if control.pending_partition_exit_stack.is_some() {
            return Err(unsupported(
                "NativeExitScopeTransitionV1",
                "source invocation crossed an unsealed partition exit stack",
            ));
        }
        control.pending_partition_exit_stack = Some(unwind);
        control.continuation = SourceContinuation::ApplyRecursorSelection {
            layer: selection,
            next: Box::new(control.continuation),
        };
        let _ = (activation, checked_ih_invocation);
        Ok(control)
    }
    ...
    fn push_selected_head_producer_kont(
        ...
        if defer_successor_until_after_selected_scope {
            if active.selected_scope.is_none() {
                return Err(unsupported(
                    "NativeProducerContinuationStepV1",
                    "scope-body split has no selected scope to apply",
                ));
            }
            let original_successor = self.active_partition_producer_kont.ok_or_else(|| {
                unsupported(
                    "NativeProducerContinuationStepV1",
                    "scope-body split has no exact body-return successor",
                )
            })?;
            let after_scope = ActiveContinuationFrame {
                selected_scope: None,
                ..active
            };
            let after_scope = self
                .push_selected_head_producer_kont(builder, after_scope, false)?
                .ok_or_else(|| {
                    unsupported(
                        "NativeProducerContinuationStepV1",
                        "scope-body split could not intern its post-scope successor",
                    )
                })?;
            self.active_partition_producer_kont = Some(after_scope);
            let selected_scope_only = ActiveContinuationFrame {
                pending: &[],
                ..active
            };
            let selected_head =
                self.push_selected_head_producer_kont(builder, selected_scope_only, false);
            self.active_partition_producer_kont = Some(original_successor);
    ...
    fn call_restored_selected_producer_kont<'b>(
        ...
        let return_tail = self
            .active_partition_return_contract
            .as_ref()
            .and_then(|contract| contract.live_producer_tail)
            .map_or(successor, |site_id| PartitionProducerKontCursor {
                site_id,
                capture_pointer: successor.capture_pointer,
            });
        let previous = self.active_partition_producer_kont.replace(successor);
        let active = selected.as_active(selected_lineage);
        let head = self
            .push_selected_head_producer_kont(builder, active, false)?
            .ok_or_else(|| {
                unsupported(
                    "NativeProducerContinuationStepV1",
        ...
        if !consumed {
            return Err(unsupported(
                "NativeProducerContinuationStepV1",
                "selected-head helper returned without exact completion evidence",
            ));
        }
        Self::complete_partition_producer_tail(
            lowered,
            return_tail,
            u64::try_from(head.site_id).map_err(|_| {
                unsupported(
                    "NativeProducerContinuationStepV1",
                    "selected-head site identity exceeds producer-tail evidence",
                )
            })?,
        )
    }

    fn call_scope_body_selected_producer_kont<'b>(
        &mut self,
        builder: &mut FunctionBuilder<'_>,
        successor: PartitionProducerKontCursor,
        value: Lowered,
        ...
        let previous = self.active_partition_producer_kont.replace(successor);
        let body_active = selected.as_active(selected_lineage);
        let head = self
            .push_selected_head_producer_kont(builder, body_active, true)?
            .ok_or_else(|| {
                unsupported(
                    "NativeProducerContinuationStepV1",
                    "scope body selected head has no explicit producer transition",
                )
            })?;
        self.active_partition_producer_kont = previous;
        let completed = self.call_partition_producer_kont(builder, head, value)?;
        let (lowered, consumed) = Self::consume_partition_producer_tail_completion(
            completed,
            return_tail,
            "scope body selected head",
        )?;
        if !consumed {
            return Err(unsupported(
                "NativeProducerContinuationStepV1",
                "scope body selected-head helper returned without exact completion evidence",
            ));
        }
        Self::complete_partition_producer_tail(
            lowered,
            ...
            u64::try_from(head.site_id).map_err(|_| {
                unsupported(
                    "NativeProducerContinuationStepV1",
                    "scope body selected-head site exceeds producer-tail evidence",
                )
            })?,
        )
    }
    ...
    fn call_partition_producer_kont(
        ...
        if matches!(&plan.action, ProducerKontAction::Done { .. }) && plan.successor.is_some() {
            return Err(unsupported(
                "NativeProducerContinuationStepV1",
                "producer terminal cannot carry a successor",
            ));
        }
        if !matches!(
            &plan.action,
            ProducerKontAction::Done { .. } | ProducerKontAction::ExitScopeComplete { .. }
        ...
        let return_producer_tail = self
            .active_partition_return_contract
            .as_ref()
            .and_then(|contract| contract.live_producer_tail)
            .or(Some(cursor.site_id));
        let mut fields = Vec::new();
    ...
    fn lower_computational_match_value_composed(
        ...
        if std::env::var_os("KEN_NATIVE_EXIT_TRACE").is_some()
            && !matches!(scrutinee, Lowered::Constructor { .. })
        {
            use std::io::Write as _;
            if let Ok(mut trace) = std::fs::OpenOptions::new()
                .create(true)
                ...
                .open("/tmp/ken-native-exit-trace")
            {
                ...
                let eliminator_kinds = eliminators
                    ...
                    .map(|frame| match frame {
                        EliminatorFrame::Computational(_) => "computational",
                        EliminatorFrame::Ordinary(_) => "ordinary",
                        EliminatorFrame::PendingLet(_) => "pending-let",
                        EliminatorFrame::Active(_) => "active",
                        EliminatorFrame::InvocationReturn => "return",
                    })
                    .collect::<Vec<_>>();
                let _ = writeln!(
                    trace,
                    "bad-match value={} eliminators={eliminator_kinds:?} producer={active_producer:?}",
    ...
    fn lower_arm_partition_work_item(
        ...
        let terminal = if let Some(producer_kont) = item.producer_kont {
            ...
            self.push_done_producer_kont(
                ...
                PartitionProducerKontTerminalIdentity::ScalarArmReturn {
                    partition_site_id: item.return_authority.descriptor.partition_site_id,
                    edge_index: item.return_authority.descriptor.edge_index,
                },
            )?
        };
        let mut return_contract = PartitionStateReturnContract::producer_terminal(
            checked_join.clone(),
            item.return_authority.descriptor.required_kind,
        );
        return_contract.live_producer_tail = Some(terminal.site_id);
        ...
        let lowered = match lowered {
            ...
            Lowered::ProcessExitStatus { .. } => {
                return Err(unsupported(
                    "NativeProducerContinuationStepV1",
                    "outlined arm returned an unaccounted scalar with a live producer tail",
                ));
            }
            value => {
                ...
                if !consumed {
                    return Err(unsupported(
                        "NativeProducerContinuationStepV1",
                        "outlined arm producer helper returned without exact completion evidence",
                    ));
                }
                lowered
            }
        ...
        match lowered {
            ...
            value => {
                ...
                let caller_tag_pointer = self.partition_output_tag_pointer.ok_or_else(|| {
                    unsupported(
                        "NativeFunctionPartition",
                        "outlined Arm final scalar return has no caller tag output",
                    )
                })?;
                self.defer_or_consume_partition_arm_return(
                    item.return_authority,
                    item.helper_index,
                    actual_kind,
                    checked_join,
    ...
    fn lower_source_arm_partition_work_item(
        ...
        Self::rebuild_partition_exit_stack_values(
    ...
    fn lower_source_kont_partition_work_item(
        ...
        if let Some(parent_return) = &mut item.resume_parent {
            parent_return.capture_pointer = loaded.next().ok_or_else(|| {
                unsupported(
                    "NativeExitScopeTransitionV1",
                    "source resume lost its selected-return cell pointer",
                )
            })?;
        } else {
            item.selected_activation_instance =
        ...
        let control = SourceControl {
            ...
            selected_lineage: if restored_selected.is_some() {
                Vec::new()
            } else {
                selected_lineage
            },
    ...
    fn restore_partition_selected_parent(
        &mut self,
        builder: &mut FunctionBuilder<'_>,
            ...
            .map(|(index, field_type)| {
                let offset = index
                    ...
                    .ok_or_else(|| {
                        unsupported(
                            "NativeExitScopeTransitionV1",
                            "selected-parent capture offset overflowed",
                        )
                    })?;
                Ok(builder.ins().load(
                    *field_type,
        ...
        if captures.next().is_some() {
            return Err(unsupported(
                "NativeExitScopeTransitionV1",
                "selected-parent source cell has trailing fields",
            ));
        }
        let SourcePrefixTemplate::ReturnFromSelectedCase {
            delimiter,
            parent_capture: Some(mut parent),
            ..
        } = definition.current
        else {
            return Err(unsupported(
        ...
        parent.cursor_instance = ControlCursorRef(builder.ins().load(
            pointer_type,
            MemFlags::trusted(),
            delimiter.cursor_instance.0,
            PARTITION_FRAME_FIELD_BYTES as i32,
        ));
        if let Some(parent_scope) = &mut parent.selected_scope {
    ...
    fn call_partition_source_eval<'b>(
        ...
        Self::append_partition_exit_stack_values(
            &control.pending_partition_exit_stack,
            &mut fields,
        );
    ...
    fn call_partition_source_resume(
        &mut self,
        builder: &mut FunctionBuilder<'_>,
        head: PartitionSourceCursor,
        parent_return: PartitionSourceCursor,
        ...
        if !partition_lowered_is_admissible(&input) {
            return Err(unsupported(
                "NativeExitScopeTransitionV1",
                "source-resume input has no exact closed schema",
            ));
        }
        let return_contract = self
            .active_partition_return_contract
            ...
            .ok_or_else(|| {
                unsupported(
                    "NativeExitScopeTransitionV1",
                    "source resume has no checked scalar return contract",
                )
            })?;
        let parent_schema = match self
            .partition_source_nodes
            ...
            .current
        {
            ...
            _ => {
                return Err(unsupported(
                    "NativeExitScopeTransitionV1",
                    "source resume parent schema is not a selected return",
                ));
            }
        };
        let mut fields = Vec::new();
        append_partition_lowered_values(self, builder, &input, &mut fields)?;
        fields.push(head.capture_pointer);
        if let Some(producer) = producer_kont {
            fields.push(producer.capture_pointer);
        }
        ...
        let key = PartitionSemanticStateKey::SourceKont(PartitionSourceKontKey::new_resume(
            ...
            pending_exit_stack
                .as_ref()
                .and_then(|stack| stack.partition_open_obligation.map(|cursor| cursor.node)),
            producer_kont.map(|cursor| cursor.site_id),
            return_contract
                .live_producer_tail
        ...
        let (state_id, state, newly_reserved) = if let Some((state_id, state)) = existing {
            ...
            let function = *self.partition_helper_ids.get(helper_index).ok_or_else(|| {
                unsupported(
                    "NativeExitScopeTransitionV1",
                    "source resume exhausted its predeclared helper pool",
                )
            })?;
            self.partition_next_helper += 1;
            let (state_id, state) = self.partition_continuations.reserve(
        ...
        let (_, function_ref) = self.partition_helper_ref(
            builder,
            state.helper_index,
            "interned source resume lost its helper identity",
        )?;
        let frame = builder.create_sized_stack_slot(StackSlotData::new(
            StackSlotKind::ExplicitSlot,
            partition_frame_size(frame_values.len())?,
        ...
        for (index, value) in frame_values.iter().copied().enumerate() {
            let offset = index
                ...
                .ok_or_else(|| {
                    unsupported(
                        "NativeExitScopeTransitionV1",
                        "source-resume input-cell store offset overflowed",
                    )
                })?;
            builder.ins().stack_store(value, frame, offset);
        }
        let invocation = self
            .invocation_pointer
            .expect("source resume owns an invocation pointer");
        let pointer_type = builder.func.dfg.value_type(invocation);
        let frame_pointer = builder.ins().stack_addr(pointer_type, frame, 0);
        let tag_output = builder.create_sized_stack_slot(StackSlotData::new(
            StackSlotKind::ExplicitSlot,
        ...
        if newly_reserved {
            self.partition_queue
                .push_back(PartitionWorkItem::SourceKont(SourceKontPartitionWorkItem {
                    ...
                    selected_cursor_instance: parent_schema.cursor_instance,
    ...
    fn lower_producer_kont_partition_work_item(
        ...
        let action_name = match &item.action {
            ...
            ProducerKontAction::CheckedComputationalIHReturn { .. } => {
                "Producer.CheckedComputationalIHReturn"
            }
            ProducerKontAction::ScopeBodyReturn { .. } => "Producer.ScopeBodyReturn",
            ProducerKontAction::ExitScopeStart { .. } => "Producer.ExitScopeStart",
            ProducerKontAction::ExitScopeComplete { .. } => "Producer.ExitScopeComplete",
        };
        if std::env::var_os("KEN_NATIVE_EXIT_TRACE").is_some() {
            use std::io::Write as _;
            if let Ok(mut trace) = std::fs::OpenOptions::new()
                ...
                .open("/tmp/ken-native-exit-trace")
            {
                ...
                let _ = writeln!(
                    ...
                    "producer-state id={} site={} action={} input={} successor={:?} \
                     return_tail={:?} pending={pending_len:?} scope={selected_scope:?} \
                     deferred={deferred_scope_successor:?}",
                    item.state_id,
                    item.site_id,
                    action_name,
                    lowered_value_kind(&item.value),
                    item.successor.map(|cursor| cursor.site_id),
            ...
            if let Some(tail_site_id) = item.return_contract.live_producer_tail {
                Self::consume_partition_producer_tail_completion(
                    ...
                    PartitionProducerKontCursor {
                        ...
                        capture_pointer: item
                            .capture_pointer
                            .expect("producer return contract capture pointer"),
                    },
                    "producer action",
                )?
            } else if matches!(lowered, Lowered::CompletedProducerTail { .. }) {
                return Err(unsupported(
                    "NativeProducerContinuationStepV1",
                    "producer-tail completion reached an empty-tail return contract",
                ));
            } else {
                (lowered, false)
            };
        let lowered = if matches!(lowered, Lowered::Trap(_)) {
            lowered
            ...
            Self::complete_partition_producer_tail(
                ...
                PartitionProducerKontCursor {
                    site_id: item
                        .return_contract
                        .live_producer_tail
                        .expect("completed producer return has an exact tail"),
                    capture_pointer: item
                        .capture_pointer
                        .expect("completed producer return has a capture pointer"),
                },
                u64::try_from(item.state_id).map_err(|_| {
                    unsupported(
                        "NativeProducerContinuationStepV1",
                        "producer return state exceeds completion evidence",
                    )
                })?,
            )?
        } else {
            match (&item.action, item.successor) {
                (ProducerKontAction::Done { .. }, None) => lowered,
                (ProducerKontAction::ExitScopeStart { .. }, Some(_)) => lowered,
                (ProducerKontAction::ExitScopeComplete { .. }, None) => lowered,
                (_, Some(successor)) => {
                    let completed =
                    ...
                    if !consumed {
                        return Err(unsupported(
                            "NativeProducerContinuationStepV1",
                            "producer successor returned without exact completion evidence",
                        ));
                    }
                    Self::complete_partition_producer_tail(
                        lowered,
                        ...
                        u64::try_from(item.state_id).map_err(|_| {
                            unsupported(
                                "NativeProducerContinuationStepV1",
                                "producer successor state exceeds completion evidence",
                            )
                        })?,
                    )?
                }
        ...
        let lowered = self.reconcile_partition_source_helper_exit(
            lowered,
            &item.return_contract,
            item.state_id,
            action_name,
                        ...
                        SourceContinuation::Terminal(
                            ...
                            if std::env::var_os("KEN_NATIVE_EXIT_TRACE").is_some() {
                                    ...
                                    .open("/tmp/ken-native-exit-trace")
                                {
                                    let _ = writeln!(
                                        trace,
                                        "return-hole stack={:?} source={:?}",
                                        stack.partition_cursor.map(|cursor| cursor.node.0),
                                        control.partition_cursor.map(|cursor| cursor.node.0),
                                    );
                                }
                            }
                            #[cfg(test)]
                        ...
                        SourceContinuation::UnwindRecursorSegment {
                            ...
                            if std::env::var_os("KEN_NATIVE_EXIT_TRACE").is_some() {
                                    ...
                                    .open("/tmp/ken-native-exit-trace")
                                {
                                    let _ = writeln!(
                                        trace,
                                        "unwind-exec stack={:?} source={:?} consumed={:?}",
                                        stack.partition_cursor.map(|cursor| cursor.node.0),
                                        control.partition_cursor.map(|cursor| cursor.node.0),
                                        control
                                            .consumed_partition_cursor
                                            .map(|cursor| cursor.node.0),
                                    );
                                }
                            }
                            let resume_active = source_active_cursor(
    ...
    fn lower_source_checked_template_predecessor<'b>(
        ...
        self.call_partition_source_predecessor(
            ...
            control.pending_partition_exit_stack.clone(),
            &control.selected,
            control.terminal_outer,
            &target,
            &selected_lineage,
    ...
    fn lower_source_dynamic_host_result_match<'b>(
        ...
        let transferred_producer_tail = self.active_partition_producer_kont;
        if let (Some(active), Some(control)) =
            (transferred_producer_tail, suffix_control.producer_kont)
        {
            if active.site_id != control.site_id
                || active.capture_pointer != control.capture_pointer
            {
                return Err(unsupported(
                    "NativeProducerContinuationStepV1",
                    "host-result fanout active tail disagrees with its Source control tail",
                ));
            }
        }
        let (source_prefix_template, terminal) =
