...
fn alphabet_equals_perform_node_signatures() {
    ...
    let alpha = EffectRow::from_effects(vec!["Console.Write".to_string(), "State.Get".to_string()]);

    // A simple program: no obligations, just the alphabet.
    let result = emit_export("h", &[], &BTreeSet::new(), alpha, vec![], vec![])
    ...
    assert!(
        !result.alphabet.contains("Net.Fetch"),
        "orphan symbol must not appear"
    );
