...
fn base_env() -> ElabEnv {
    ...
    env.elaborate_ken_md_file(TRANSPORT_KEN_MD)
        .expect("Core/Transport.ken.md must elaborate");
    env
}
...
fn all_laws_kernel_check_before_literate_examples() {
    let extracted = ken_elaborator::literate::extract_ken_md(NAT_ARITH_KEN_MD)
        .expect("NatArith.ken.md must extract");
...
fn entry_elaborates_and_registers_the_free_laws() {
    ...
    env.elaborate_ken_md_file(NAT_ARITH_KEN_MD)
        .expect("Core/NatArith.ken.md must elaborate and kernel-check");
    for name in [
        "add",
        "mul",
        "add_assoc",
...
fn canonical_operations_compute_on_concrete_naturals() {
    let mut env = base_env();
    env.elaborate_ken_md_file(NAT_ARITH_KEN_MD)
        .expect("Core/NatArith.ken.md must elaborate");
    env.elaborate_decl(
        "const add_two_three : Equal Nat (add (Suc (Suc Zero)) (Suc (Suc (Suc Zero)))) (Suc (Suc (Suc (Suc (Suc Zero))))) = tt",
    )
    .expect("add 2 3 must compute to 5");
    env.elaborate_decl(
        "const mul_two_three : Equal Nat (mul (Suc (Suc Zero)) (Suc (Suc (Suc Zero)))) (Suc (Suc (Suc (Suc (Suc (Suc Zero)))))) = tt",
    )
