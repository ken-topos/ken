...
//! `packages/transport/transport.ken` source (producer-grep: drives the
...
fn j_elaborates_to_a_real_term_j_node() {
    ...
    let ids = env
        .elaborate_file(
            "view refl_test (ty : Type) (a : ty) (q : Equal ty a a) : Equal ty a a = \
               J (\\b' _. Equal ty a b') Refl q",
        )
        .expect("J on refl should elaborate and kernel-check");
    let id = ids[0];
    let (_, body) = env.env.transparent_body(id).expect("view must be transparent");
    assert!(
        mentions_j(&body),
...
fn ill_typed_transport_wrong_equation_is_kernel_rejected() {
    ...
    let res = env.elaborate_file(
        "view bad_transport (ty : Type) (a : ty) (b : ty) (q : Equal ty a b) \
...
fn ill_typed_transport_wrong_witness_type_is_kernel_rejected() {
    ...
    let res = env.elaborate_file(
        "view not_an_eq (ty : Type) (a : ty) (not_a_proof : ty) : ty = \
...
fn transport_package_adds_zero_trusted_base_delta() {
    ...
    assert_eq!(
        base_tb, with_pkg_tb,
        "loading packages/transport/transport.ken must add ZERO trusted_base() entries \
...
fn stuck_match_over_abstract_key_transports_via_hand_written_motive() {
    ...
    let ids = env
        .elaborate_file(
            "view stuck_of (k : Bool) : Bool = match k { True => True ; False => False }\n\
             view stuck_transport (k : Bool) (q : Equal Bool True k) \
    ...
    let (_, body) = env
        ...
        .expect("view must be transparent");
...
fn stuck_match_transports_via_package_sym() {
    ...
    let ids = env
        .elaborate_file(
            "view stuck_of2 (k : Bool) : Bool = match k { True => True ; False => False }\n\
             view stuck_transport2 (k : Bool) (q : Equal Bool k True) \
