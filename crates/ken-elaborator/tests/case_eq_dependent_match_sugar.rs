    ...
    let ids = env
        .elaborate_file(
            "fn da_eq (x : Bool) (y : Bool) : Bool = x\n\
             theorem stuck_eq (x : Bool) (y : Bool) : Equal Bool (da_eq x y) (da_eq x y) = match (da_eq x y) eqn: h {
...
fn three_constructor_enum_is_not_bool_hardcoded() {
    ...
    env.elaborate_file(
        "data OrdResult = Lt | Eq | Gt\n\
         theorem ord_self (o : OrdResult) : Equal OrdResult o o = match o eqn: h {
...
fn reverted_hypothesis_shape_elaborates_with_a_real_byte_reduction() {
    ...
    const SUGAR: &str = "fn cons_eq (x : Bool) (y : Bool) : Bool = x\n\
        ...
        theorem cons_sound (x : Bool) (y : Bool) : IsTrueLike (cons_eq x y) -> Equal Bool x x = \
    ...
    const HAND_IDIOM: &str = "theorem cons_sound (x : Bool) (y : Bool) : IsTrueLike (cons_eq x y) -> Equal Bool x x = \
        bool_dichotomy (cons_eq x y) |> dispatch_cons_eq x y where \
        theorem dispatch_cons_eq (x : Bool) (y : Bool) (d : Or (Equal Bool (cons_eq x y) True) (Equal Bool (cons_eq x y) False)) \
...
fn non_nullary_constructors_are_rejected_at_the_modifier_boundary() {
    ...
    let result = env.elaborate_file(
        "data Parcel = Wrap Bool\n\
         theorem no_fields (b : Parcel) : Equal Parcel b b = match b eqn: h { Wrap x |-> Refl }",
    );
    assert!(
...
fn modifier_has_zero_trusted_base_delta() {
    ...
    let mut env = mk_env();
    env.elaborate_file(
        "theorem eqn_tb (b : Bool) : Equal Bool b b = match b eqn: h {
           True |-> J (\\b' _. Equal Bool b' b') Refl h ;
           False |-> J (\\b' _. Equal Bool b' b') Refl h }",
    )
