//! V0 parser: token stream → surface AST (`32 §8`, `39 §5.2`).
//!
//! Recursive descent, no backtracking beyond the fixed Pi-lookahead.
//! Precedences (lowest to highest): ascription `:` < arrow `->` < application.
...
impl Parser {
    ...
    /// Look `n` tokens ahead (does not consume).
    ...
    /// Accept a lowercase or uppercase identifier as a binder name.
    ...
    /// Parse a sequence of top-level declarations until EOF.
    ...
    fn parse_decl(&mut self) -> Result<Decl, ElabError> {
        ...
        match self.peek().clone() {
            Token::KwView => self.parse_view_decl(start),
            Token::KwLet => self.parse_let_decl(start),
            other => Err(ElabError::ParseError {
                msg: format!("expected 'view' or 'let', found {:?}", other),
                span: self.peek_span().clone(),
            }),
        }
    }

    fn parse_view_decl(&mut self, start: usize) -> Result<Decl, ElabError> {
        self.advance(); // consume 'view'
        let (name, _) = self.expect_ident()?;

        // Parse binders: one or more `( name+ : type )` (name = Ident | ConId)
        let mut params = Vec::new();
        while matches!(self.peek(), Token::LParen)
            && matches!(self.lookahead(1), Token::Ident(_) | Token::ConId(_))
        {
            // Check it's a binder: `( ident+ : ...` vs `( type )` used as atom
            // A binder has ident(s) followed by `:`.  We need a lookahead: after
            // `(`, walk idents until we see `:` or something else.
    ...
    /// Returns true if the next tokens form `( name+ : …`, i.e. a binder.
    fn is_binder_ahead(&self) -> bool {
        if !matches!(self.peek(), Token::LParen) {
            return false;
        }
        // Walk past `(` and any lowercase/uppercase names to see if `:` follows
        let mut i = 1;
        while matches!(self.lookahead(i), Token::Ident(_) | Token::ConId(_)) {
            i += 1;
        }
        // After one or more names, must see `:` for a binder
    ...
    /// Returns true if the upcoming tokens look like `( name : type ) -> …`.
    ...
    fn parse_dep_pi(&mut self) -> Result<Type, ElabError> {
        ...
        let b = self.parse_type()?; // codomain (right-assoc)
        let end = b.span().end;
        Ok(Type::TPi(x, Box::new(a), Box::new(b), Span::new(start, end)))
    }

    /// Parse an atom type: `Type level?`, `ConId`, `ident` (type var), or `(type)`.
    fn parse_atom_type(&mut self) -> Result<Type, ElabError> {
        ...
        match self.peek().clone() {
            ...
            // Both uppercase (ConId) and lowercase (Ident) are treated as
            // names to resolve; the resolver distinguishes scope vs. globals.
            ...
            Token::LParen => {
                // Check if it's a Pi `(ident : type) -> type` — if so, caller
                // should have called parse_dep_pi already; this `(` starts a
                // parenthesised type.
                self.advance(); // consume `(`
                let ty = self.parse_type()?;
                self.expect(&Token::RParen)?;
    ...
    /// Parse a full expression (with optional `: type` ascription at the end).
    pub fn parse_expr(&mut self) -> Result<Expr, ElabError> {
        ...
        if matches!(self.peek(), Token::Colon) {
            ...
            // Absorb the colon into an ascription; the `_colon_span` is for
            // the span merge above.
            let _ = colon_span;
    ...
    /// Parse an application spine or a lambda/let.
    fn parse_app_expr(&mut self) -> Result<Expr, ElabError> {
        match self.peek().clone() {
            Token::Lambda => self.parse_lambda(),
            Token::KwLet => self.parse_let_expr(),
            _ => {
                // Application: parse atoms left-to-right
                let mut f = self.parse_atom_expr()?;
                loop {
                    // An atom expression starts with: ident, ConId, Type, `(`
                    // (and not `->` or `:` or `=` or `.` or `)`
    ...
    fn parse_lambda(&mut self) -> Result<Expr, ElabError> {
        ...
        // Collect one or more binder names before the `.` (Ident or ConId)
        let mut names = Vec::new();
        loop {
            match self.peek().clone() {
    ...
    fn parse_let_expr(&mut self) -> Result<Expr, ElabError> {
        ...
        self.expect(&Token::Eq)?;
        let rhs = self.parse_app_expr()?; // rhs does NOT absorb 'in'
    ...
    /// Parse an atom expression: a variable, ConId, `Type`, or a parenthesized
    /// expression (which may itself contain an ascription).
    fn parse_atom_expr(&mut self) -> Result<Expr, ElabError> {
        ...
        match self.peek().clone() {
            ...
            Token::LParen => {
                self.advance(); // consume `(`
                // Parse an expression inside (which can have `: type` ascription)
                let inner = self.parse_expr()?;
                self.expect(&Token::RParen)?;
                // The parens are transparent: return the inner expression,
                // updating its span to include the parens.
                let end = self.tokens[self.pos - 1].1.end;
                // Re-wrap with the paren span so Span::merge is accurate
                let span = Span::new(start, end);
                Ok(match inner {
                    Expr::EAsc(e, t, _) => Expr::EAsc(e, t, span),
                    e => {
                        // Just return the inner expr but with the paren span
                        // as a wider span. We'll re-box if needed.
                        match e {
    ...
    /// Parse a single standalone expression (for tests).
...
/// Parse a program (sequence of declarations) from source text.
