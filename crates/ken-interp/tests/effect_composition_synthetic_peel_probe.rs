...
use ken_interp::{run_io, CaptureHost, ConsoleIds, CoproductIds, EvalStore, EvalVal, RunIoError};
...
fn wrap(id: GlobalId, coproduct_ids: &CoproductIds, on_left: bool) -> EvalVal {
    let tag = if on_left {
        coproduct_ids.inl_id
    } else {
        coproduct_ids.inr_id
    };
    let inner = EvalVal::Ctor {
        id,
        args: Rc::new(vec![]),
        slot: 0,
    };
...
fn single_wrap_peels_to_the_synthetic_base_tag() {
    ...
    let coproduct_ids = CoproductIds {
        inl_id: elab.prelude_env.inl_id,
        inr_id: elab.prelude_env.inr_id,
    };
    let console_ids = ConsoleIds::from_elab(&elab).expect("Console ABI");
    ...
    match result {
        Err(RunIoError::UnknownEffect(EvalVal::Ctor { id, args, .. })) => {
            assert_eq!(
                id, ping_id,
                "the peel must land exactly on the synthetic Ping tag"
            );
            assert!(
                args.is_empty(),
                "Ping is nullary — no stray wrapper args should survive the peel"
            );
        }
...
fn nested_wrap_peels_through_multiple_layers_to_the_synthetic_base_tag() {
    ...
    match result {
        Err(RunIoError::UnknownEffect(EvalVal::Ctor { id, .. })) => {
            assert_eq!(
                id, ping_id,
                "the peel must recurse through BOTH InR/InL layers to reach Ping"
            );
        }
        other => panic!("expected UnknownEffect(Ping); got {other:?}"),
    }
...
fn no_coproduct_ids_disables_peeling() {
    ...
    match result {
        Err(RunIoError::UnknownEffect(EvalVal::Ctor { id, .. })) => {
            assert_eq!(
                id, coproduct_ids.inl_id,
                "with coproduct_ids=None the wrapper must reach dispatch UNPEELED"
            );
        }
