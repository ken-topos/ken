...
//! `Sum`-peel routes an ARBITRARY base tag, closing the structural leg's
...
use ken_interp::{run_io, ConsoleIds, EvalStore, EvalVal, RunIoError, SumIds};
...
fn wrap(id: GlobalId, sum_ids: &SumIds, on_left: bool) -> EvalVal {
    let tag = if on_left { sum_ids.inl_id } else { sum_ids.inr_id };
...
fn single_wrap_peels_to_the_synthetic_base_tag() {
    ...
    let sum_ids = SumIds { inl_id: elab.prelude_env.inl_id, inr_id: elab.prelude_env.inr_id };
    ...
    let op = wrap(ping_id, &sum_ids, true);
    ...
    let result = run_io(vis, &console_ids, None, Some(&sum_ids), &elab.env, &mut store);
    match result {
...
fn nested_wrap_peels_through_multiple_layers_to_the_synthetic_base_tag() {
    ...
    let inner = wrap(ping_id, &sum_ids, true); // InL Ping
    let op = {
...
/// `sum_ids = None` disables peeling entirely (BV6: pre-composition callers
/// unaffected) — a wrapped op with no `SumIds` supplied is dispatched
...
fn no_sum_ids_disables_peeling() {
    ...
    match result {
        Err(RunIoError::UnknownEffect(EvalVal::Ctor { id, .. })) => {
            assert_eq!(id, sum_ids.inl_id, "with sum_ids=None the wrapper must reach dispatch UNPEELED");
        }
