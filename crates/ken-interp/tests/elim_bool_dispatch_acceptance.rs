...
fn computed_bool_true_dispatches_to_first_method() {
    let result = eval_view(
        "view t = match (eq_int 5 5) { True => 1 ; False => 2 }",
    );
...
fn computed_bool_false_dispatches_to_second_method() {
    let result = eval_view(
        "view t = match (eq_int 5 6) { True => 1 ; False => 2 }",
    );
...
fn computed_bool_agrees_with_literal_bool_dispatch() {
    let computed_true = eval_view("view t = match (eq_int 5 5) { True => 1 ; False => 2 }");
    let literal_true = eval_view("view t = match True { True => 1 ; False => 2 }");
    assert_eq!(computed_true, literal_true, "computed True and literal True must agree");

    let computed_false = eval_view("view t = match (eq_int 5 6) { True => 1 ; False => 2 }");
    let literal_false = eval_view("view t = match False { True => 1 ; False => 2 }");
...
fn computed_bool_via_leq_int_dispatches_correctly() {
    let le = eval_view("view t = match (leq_int 3 5) { True => 1 ; False => 2 }");
    assert_eq!(le, EvalVal::Int(1), "leq_int 3 5 is True — must select methods[0]");
    let gt = eval_view("view t = match (leq_int 5 3) { True => 1 ; False => 2 }");
