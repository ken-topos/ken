...
fn absurd_discharges_type_motive_from_constructor_disjoint_hypothesis() {
    let (mut env, b) = mk_env();
    let n = declare_postulate(&mut env, "test postulate".to_string(), vec![], nat_t(&b))
        .expect("n : Nat");
    ...
    let p_zero_suc = declare_postulate(
        &mut env,
        "test postulate".to_string(),
        vec![],
        p_zero_suc_ty,
    )
    .expect("p : Eq Nat Zero (Suc n)");
...
fn neutral_index_equality_cannot_feed_type_absurd() {
    let (mut env, b) = mk_env();
    let x = declare_postulate(&mut env, "test postulate".to_string(), vec![], nat_t(&b))
        .expect("x : Nat");
    let y = declare_postulate(&mut env, "test postulate".to_string(), vec![], nat_t(&b))
        .expect("y : Nat");
    ...
    let p_neutral = declare_postulate(&mut env, "test postulate".to_string(), vec![], p_neutral_ty)
        .expect("p : Eq Nat x y");
...
fn sct_rejects_self_reference_laundered_through_type_absurd_motive() {
    let mut env = GlobalEnv::new();
    let bottom_ty = bottom_term(&env);
    let impossible = declare_postulate(&mut env, "test postulate".to_string(), vec![], bottom_ty)
        .expect("p : Bottom for unreachable code");
    let result =
